#!/usr/bin/env bash
#
# jql/mayhem/build.sh — build yamafaktory/jql's cargo-fuzz target as a sanitized libFuzzer
...
#
# jql is a pure-Rust JSON query workspace (jql-parser / jql-runner / jql CLI). Upstream ships
# its own cargo-fuzz crate in fuzz/ with one target:
#   fuzz_parser — feeds arbitrary UTF-8 to jql_parser::parser::parse() (the query parser).
# The old fork's Mayhem target (`fuzz-parser`, harness fuzz/fuzz_targets/fuzz_parser.rs) drove
# the exact same entry point; this build keeps it.
#
...
# Upstream's cargo-fuzz crate lives in fuzz/ (cargo-fuzz convention).
...
ls -la /mayhem/fuzz_parser 2>&1 || true
